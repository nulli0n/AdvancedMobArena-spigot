...
public class PlayerList<P extends IArenaPlayer> {
    ...
    public Set<P> players() {
        return this.players.stream().filter(Predicate.not(P::isSpectator)).collect(Collectors.toSet());
    }

    @NotNull
    public Set<P> select(@NotNull PlayerType playerType) {
        return this.players.stream().filter(arenaPlayer -> !arenaPlayer.isSpectator() && arenaPlayer.getType() == playerType).collect(Collectors.toSet());
    }

    @NotNull
    public Set<P> select(@NotNull GameState state) {
        return this.players.stream().filter(arenaPlayer -> arenaPlayer.getState() == state && !arenaPlayer.isSpectator()).collect(Collectors.toSet());
    }
