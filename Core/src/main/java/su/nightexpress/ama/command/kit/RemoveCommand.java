...
class RemoveCommand extends AbstractCommand<AMA> {

    public RemoveCommand(@NotNull AMA plugin) {
        super(plugin, new String[]{"remove"}, Perms.COMMAND_KIT);
        this.setDescription(plugin.getMessage(Lang.COMMAND_KIT_REMOVE_DESC));
        this.setUsage(plugin.getMessage(Lang.COMMAND_KIT_REMOVE_USAGE));
    }
    ...
    protected void onExecute(@NotNull CommandSender sender, @NotNull CommandResult result) {
        ...
        plugin.getUserManager().getUserDataAndPerformAsync(result.getArg(2), user -> {
            ...
            if (user.removeKit(kit)) {
                plugin.getUserManager().saveUser(user);
            }

            this.plugin.getMessage(Lang.COMMAND_KIT_REMOVE_DONE)
