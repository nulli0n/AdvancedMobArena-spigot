...
public class ArenaDataHandler extends AbstractUserDataHandler<AMA, ArenaUser> {
    ...
    private static ArenaDataHandler instance;

    private final Function<ResultSet, ArenaUser> userFunction;

    protected ArenaDataHandler(@NotNull AMA plugin) {
        super(plugin, plugin);
    ...
    @NotNull
    public static synchronized ArenaDataHandler getInstance(@NotNull AMA plugin) throws SQLException {
        if (instance == null) {
            instance = new ArenaDataHandler(plugin);
        }
        return instance;
    }
