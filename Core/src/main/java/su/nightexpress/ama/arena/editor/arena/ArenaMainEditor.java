...
public class ArenaMainEditor extends AbstractEditorMenu<AMA, ArenaConfig> {

    public ArenaMainEditor(@NotNull AMA plugin, @NotNull ArenaConfig config) {
        super(plugin, config, ArenaEditorHub.TITLE_ARENA_EDITOR, 45);
        ...
        MenuClick click = (player, type, e) -> {
            if (type instanceof MenuItemType type2) {
                ...
                switch (type2) {
                    ...
                    case ARENA_OPEN_SUPPLY_MANAGER -> config.getSupplyManager().getEditor().open(player, 1);
    ...
    public void setTypes(@NotNull Map<EditorButtonType, Integer> map) {
        ...
        map.put(ArenaEditorType.ARENA_OPEN_SUPPLY_MANAGER, 34);
    ...
    public void onItemPrepare(@NotNull Player player, @NotNull MenuItem menuItem, @NotNull ItemStack item) {
        ...
        if (!(menuItem.getType() instanceof ArenaEditorType type)) return;

        UnaryOperator<String> replacer;
        if (type == ArenaEditorType.ARENA_OPEN_GAMEPLAY_MANAGER) {
            replacer = this.object.getGameplayManager().replacePlaceholders();
        }
        else if (type == ArenaEditorType.ARENA_OPEN_REGION_MANAGER) {
            replacer = this.object.getRegionManager().replacePlaceholders();
        }
        else if (type == ArenaEditorType.ARENA_OPEN_SHOP_MANAGER) {
            replacer = this.object.getShopManager().replacePlaceholders();
        }
        else if (type == ArenaEditorType.ARENA_OPEN_SPOT_MANAGER) {
            replacer = this.object.getSpotManager().replacePlaceholders();
        }
        else if (type == ArenaEditorType.ARENA_OPEN_WAVE_MANAGER) {
            replacer = this.object.getWaveManager().replacePlaceholders();
        }
        else if (type == ArenaEditorType.ARENA_OPEN_REWARD_MANAGER) {
            replacer = this.object.getRewardManager().replacePlaceholders();
        }
        else {
            replacer = this.object.replacePlaceholders();
            if (type == ArenaEditorType.ARENA_CHANGE_ACTIVE) {
